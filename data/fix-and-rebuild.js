// Append grades 7a-9b to math-data.js
var fs = require('fs');
var vm = require('vm');

// Read HEAD version
var head = fs.readFileSync('data/math-data-head.js', 'utf8');
fs.writeFileSync('data/math-data.js', head);
...
function parseMathData(code) {
  ...
  if (idx === -1) throw new Error('Cannot find MATH_BY_GRADE');
...
var mathData = parseMathData(code);
...
for (var grade in newGrades) {
  mathData[grade] = newGrades[grade];
}

console.log('All grades:', Object.keys(mathData).sort().join(', '));

// Serialize back to JavaScript code
function serializeObj(obj, indent) {
  ...
  keys.forEach(function(k, idx) {
    var v = obj[k];
    var keyStr = isNaN(k) ? '"' + k.replace(/"/g, '\\"') + '"' : k;
    if (typeof v === 'string') {
      lines.push(indent + '  ' + keyStr + ': "' + v.replace(/\\/g, '\\\\').replace(/"/g, '\\"') + '"');
    } else if (typeof v === 'number' || typeof v === 'boolean') {
      lines.push(indent + '  ' + keyStr + ': ' + v);
    } else if (Array.isArray(v)) {
      lines.push(indent + '  ' + keyStr + ': [' + v.map(function(item) {
        if (typeof item === 'string') return '"' + item.replace(/\\/g, '\\\\').replace(/"/g, '\\"') + '"';
        if (typeof item === 'number') return item;
        return JSON.stringify(item).replace(/\\n/g, '\\n').replace(/\\"/g, '\\"');
      }).join(', ') + ']');
    } else if (typeof v === 'object' && v !== null) {
      lines.push(indent + '  ' + keyStr + ': ' + serializeObj(v, indent + '  '));
    }
  });
  lines.push(indent + '}');
  return lines.join('\n');
...
var result = 'window.MATH_BY_GRADE = ' + JSON.stringify(mathData, null, 2).replace(/\\n/g, '\n') + ';\n';

// Wait, this won't work with the original format. Let me try a different approach.
// Just write the JSON.stringify output directly
result = 'window.MATH_BY_GRADE = ' + JSON.stringify(mathData, null, 2) + ';\n';
...
// Verify syntax
try {
  var verifyCtx = vm.createContext({});
  vm.runInContext(result, verifyCtx);
  console.log('Syntax OK');
  console.log('Grades:', Object.keys(verifyCtx.MATH_BY_GRADE).sort().join(', '));
  var total = 0;
  Object.keys(verifyCtx.MATH_BY_GRADE).forEach(function(g) { total += verifyCtx.MATH_BY_GRADE[g].problems.length; });
